...
<div class="page-header">
    ...
    <h1 class="page-title">Edit Street</h1>
    <p class="page-subtitle">Update street information</p>
</div>
...
<form method="POST" action="{{ route('streets.update', $street) }}" id="streetForm">
    @csrf
    @method('PUT')
    ...
    <div class="form-card">
        <div class="form-section">
            <div class="section-header">
                ...
                <div>
                    ...
                    <p class="section-description">Update the street details</p>
                </div>
            </div>
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <select id="society_id" name="society_id" required>
                        ...
                        @foreach($societies as $society)
                            <option value="{{ $society->id }}" {{ old('society_id', $street->block->society_id) == $society->id ? 'selected' : '' }}>
                                {{ $society->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select id="type" name="type">
                        ...
                        <option value="main" {{ old('type', $street->type) == 'main' ? 'selected' : '' }}>Main</option>
                        <option value="commercial" {{ old('type', $street->type) == 'commercial' ? 'selected' : '' }}>Commercial</option>
                        <option value="residential" {{ old('type', $street->type) == 'residential' ? 'selected' : '' }}>Residential</option>
                        <option value="side" {{ old('type', $street->type) == 'side' ? 'selected' : '' }}>Side Street</option>
                    </select>
                    @error('type')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" required>
                        ...
                        <option value="active" {{ old('status', $street->status) == 'active' ? 'selected' : '' }}>Active</option>
                        <option value="inactive" {{ old('status', $street->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                    </select>
                    @error('status')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                ...
                <div class="form-group full-width">
                    ...
                    <textarea id="description" name="description" rows="4"
                              placeholder="Enter street description">{{ old('description', $street->description) }}</textarea>
                    @error('description')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                </div>
        ...
        <div class="form-actions">
            ...
            <button type="submit" class="btn btn-primary">
                <i class="fas fa-save"></i> Update Street
            </button>
        </div>
    </div>
</form>
...
const selectedBlockId = "{{ old('block_id', $street->block_id) }}";
